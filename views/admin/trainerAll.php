...
<?php $cid = strip_tags((trim($_GET['cid2']))); ?>
...
<h4> Trainer Details </h4>
  ...
  if ($tag == '1') {
	...
	echo '<div class="report"> Allocated Client to Trainer</div>';
 ...
 if ($tag == '2') {
   ...
   echo '<div class="error"> Please contact the administrator. <br />
	...
	echo '<div class="report"> Client re-allocated to different Trainer</div>';
...
<h4> Update Trainer Allocation to Client </h4>		
Please select Client and then Trainer, then submit button will appear ready for you to submit the form.
<form>
<select name='cid2' onchange='this.form.submit()'>
<option value= ""> Select Client </option> 
<?php
$conn = mysql_connect("cloudapps.cbkmdimoancr.us-west-2.rds.amazonaws.com", "user", "password13") or die (mysql_error());
			mysql_select_db ("cloudapps");
...
$conn = mysql_connect("cloudapps.cbkmdimoancr.us-west-2.rds.amazonaws.com", "user", "password13") or die (mysql_error());
			mysql_select_db ("cloudapps");
$sql22="SELECT trainer_id, trainer_name FROM trainer
	        WHERE trainer_id not in(SELECT tc.trainer_id
			                              FROM trainer t, client c, trainer_client tc
				                              WHERE c.client_id = tc.client_id
					                              AND t.trainer_id = tc.trainer_id
                                        	AND c.client_id = $cid)";
